                            ...
                            <el-tabs type="border-card">
                                ...
                                <el-tab-pane label="行为沙箱" @click.native="showDrawer">
                                    <el-row :gutter="20" style="display: flex; justify-content: flex-start;">
                                        <el-col :span="20" style="order: 1;" @click.native.stop>
                                            <canvas ref="lineChart" width="1500" height="400">
                                            </canvas>
                                        </el-col>
                                    </el-row>
                                </el-tab-pane>
...
export default {
    ...
    methods: {
        mergeChildren(data) {
            ...
            return data.reduce((acc, item) => {
                if (item.children && item.children.length > 0) {
                    let existingItem = acc.find((el) => el.name === item.name);
                    if (!existingItem) {
                        acc.push({
                            name: item.name,
                            children: this.mergeChildren(item.children)
                        });
                    } else {
        ...
        formatData() {
            let tempdata = this.sandbox_data.data.map(item => {
                const actionName = item.action;
                const actionChildren = item.detail.map(detailItem => {
                    return {
                        name: detailItem,
                        children: [{ name: item.timestamp }]
                    };
                });
                return {
                    name: actionName,
            ...
            let mergedData = this.mergeChildren(tempdata);
            this.treedata.children = mergedData;

        },
        initwebsocket() {
            this.websocket = new WebSocket('ws://127.0.0.1:8080/api/getinfo'); // 替换成你的后端 WebSocket 地址
            ...
            this.websocket.onmessage = (event) => {
                ...
                this.network_data.detail = data.network_data;
        ...
        initPieChart() {
            if (this.status.filetype === 'evil') {
                ...
                if (typeof this.permission_data.data !== 'undefined' && this.permission_data.data !== null) {
                    infoCount["权限"] = this.permission_data.data.length;
                }
                if (typeof this.network_data.detail !== 'undefined' && this.network_data.detail !== null) {
                    infoCount["网络"] = this.network_data.detail.length;
                }
                if (typeof this.vulsandbox_data.data !== 'undefined' && this.vulsandbox_data.data !== null) {
                    infoCount["漏洞"] = this.vulsandbox_data.data.length;
                }
                if (typeof this.sandbox_data.data !== 'undefined' && this.sandbox_data.data !== null) {
                    infoCount["行为"] = this.sandbox_data.data.length;
                }

                const pieChartData = [];
                for (const [type, count] of Object.entries(infoCount)) {
                    pieChartData.push({ value: count, name: type });
                }
        ...
        typeTwoCount() {
            if (this.permission_data.data !== null) {
                ...
                this.cardcount_data.permission_data[0].targetValue = 0;
                this.cardcount_data.permission_data[1].targetValue = 0;
            }
        ...
        Getsfiletypetatus() {
            ...
            } else if (this.permission_data.data !== null && this.permission_data.data.some(item => item.type == '2')) {
